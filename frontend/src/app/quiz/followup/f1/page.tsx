...
export default function FollowUpF1() {
  ...
  const loadFollowUpQuestion = async () => {
    try {
      ...
      // Get the first question (f1)
      const f1Question = questions.find((q: FollowUpQuestion) => q.id === 'f1')
      if (!f1Question) {
        throw new Error('Question f1 not found')
      }
      
      setQuestion(f1Question)
      
      // Initialize answer with default value
      if (f1Question.type === 'multiple_choice' && f1Question.options && f1Question.options.length > 0) {
        setAnswerValue(f1Question.options[0])
      } else if (f1Question.type === 'slider' && f1Question.min_value !== undefined && f1Question.max_value !== undefined) {
        setAnswerValue(Math.round((f1Question.min_value + f1Question.max_value) / 2))
      } else if (f1Question.type === 'text_input') {
        setAnswerValue('')
      } else if (f1Question.type === 'ranking' && f1Question.items) {
        setAnswerValue([...f1Question.items]) // Default order
      }
      ...
      setAnswerValue("I've done some research")
    } finally {
      setIsLoading(false)
    }
  }
  ...
  return (
    <QuestionLayout
      questionId={1}
      title={question.text}
      description="Followup question 1 of 7"
      onNext={handleNext}
