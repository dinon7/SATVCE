...
export default function FollowUpF3() {
  ...
  const loadFollowUpQuestion = async () => {
    try {
      ...
      // Get the third question (f3)
      const f3Question = questions.find((q: FollowUpQuestion) => q.id === 'f3')
      if (!f3Question) {
        throw new Error('Question f3 not found')
      }
      
      setQuestion(f3Question)
      
      // Initialize answer with default value
      if (f3Question.type === 'multiple_choice' && f3Question.options && f3Question.options.length > 0) {
        setAnswerValue(f3Question.options[0])
      } else if (f3Question.type === 'slider' && f3Question.min_value !== undefined && f3Question.max_value !== undefined) {
        setAnswerValue(Math.round((f3Question.min_value + f3Question.max_value) / 2))
      } else if (f3Question.type === 'text_input') {
        setAnswerValue('')
      } else if (f3Question.type === 'ranking' && f3Question.items) {
        setAnswerValue([...f3Question.items]) // Default order
      }
  ...
  return (
    <QuestionLayout
      questionId={3}
      title={question.text}
      description="Followup question 3 of 7"
      onNext={handleNext}
      ...
      <div className="space-y-6">
        ...
        <div className="flex justify-between">
          ...
          <div className="text-sm text-gray-500">
            {isSubmitting ? "Processing..." : "3 of 7"}
          </div>
        </div>
      </div>
    </QuestionLayout>
