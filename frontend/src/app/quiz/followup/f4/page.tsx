...
export default function FollowUpF4() {
  ...
  const loadFollowUpQuestion = async () => {
    try {
      ...
      // Get the fourth question (f4)
      const f4Question = questions.find((q: FollowUpQuestion) => q.id === 'f4')
      if (!f4Question) {
        throw new Error('Question f4 not found')
      }
      
      setQuestion(f4Question)
      
      // Initialize answer with default value
      if (f4Question.type === 'multiple_choice' && f4Question.options && f4Question.options.length > 0) {
        setAnswerValue(f4Question.options[0])
      } else if (f4Question.type === 'slider' && f4Question.min_value !== undefined && f4Question.max_value !== undefined) {
        setAnswerValue(Math.round((f4Question.min_value + f4Question.max_value) / 2))
      } else if (f4Question.type === 'text_input') {
        setAnswerValue('')
      } else if (f4Question.type === 'ranking' && f4Question.items) {
        setAnswerValue([...f4Question.items]) // Default order
      }
      ...
      setAnswerValue(["Job security", "High salary", "Work-life balance", "Making a difference"])
    } finally {
      setIsLoading(false)
    }
  }
  ...
  return (
    <QuestionLayout
      questionId={4}
      title={question.text}
      description="Followup question 4 of 7"
      onNext={handleNext}
      ...
      <div className="space-y-6">
        ...
        <div className="flex justify-between">
          ...
          <div className="text-sm text-gray-500">
            {isSubmitting ? "Processing..." : "4 of 7"}
          </div>
        </div>
      </div>
    </QuestionLayout>
