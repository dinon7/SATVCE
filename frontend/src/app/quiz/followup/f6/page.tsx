...
export default function FollowUpF6() {
  ...
  const loadFollowUpQuestion = async () => {
    try {
      ...
      // Get the sixth question (f6)
      const f6Question = questions.find((q: FollowUpQuestion) => q.id === 'f6')
      if (!f6Question) {
        throw new Error('Question f6 not found')
      }
      
      setQuestion(f6Question)
      
      // Initialize answer with default value
      if (f6Question.type === 'multiple_choice' && f6Question.options && f6Question.options.length > 0) {
        setAnswerValue(f6Question.options[0])
      } else if (f6Question.type === 'slider' && f6Question.min_value !== undefined && f6Question.max_value !== undefined) {
        setAnswerValue(Math.round((f6Question.min_value + f6Question.max_value) / 2))
      } else if (f6Question.type === 'text_input') {
        setAnswerValue('')
      } else if (f6Question.type === 'ranking' && f6Question.items) {
        setAnswerValue([...f6Question.items]) // Default order
      }
  ...
  return (
    <QuestionLayout
      questionId={6}
      title={question.text}
      description="Followup question 6 of 7"
      onNext={handleNext}
      ...
      <div className="space-y-6">
        ...
        <div className="flex justify-between">
          ...
          <div className="text-sm text-gray-500">
            {isSubmitting ? "Processing..." : "6 of 7"}
          </div>
        </div>
      </div>
    </QuestionLayout>
