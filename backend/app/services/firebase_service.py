...
class FirebaseService:
    ...
    async def store_ai_cache(self, cache_key: str, data: Dict[str, Any], ttl: int = 3600) -> None:
        ...
        try:
            ...
            doc_data = {
                'data': data,
                'createdAt': datetime.utcnow(),
                'expiresAt': datetime.utcnow().timestamp() + ttl,
                'type': 'ai_cache'
            }
            
            # Store in Firestore
            self.db.collection('ai_cache').document(cache_key).set(doc_data)
            logger.info(f"AI cache stored for key: {cache_key}")
        except Exception as e:
    ...
    async def get_ai_cache(self, cache_key: str) -> Optional[Dict[str, Any]]:
        """
        Get AI cache data if not expired.
        ...
        """
        try:
            doc = self.db.collection('ai_cache').document(cache_key).get()
            if not doc.exists:
                return None
            
            doc_data = doc.to_dict()
            if doc_data.get('expiresAt', 0) < datetime.utcnow().timestamp():
                # Cache expired, delete it
                self.db.collection('ai_cache').document(cache_key).delete()
                return None
    ...
    async def clear_ai_cache(self, user_id: str) -> None:
        ...
        try:
            ...
            raise FirebaseError(f"AI cache clear failed: {str(e)}", "CLEAR_FAILED")
