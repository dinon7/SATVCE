...
class IntegrationService:
    ...
    async def sync_external_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Sync external data with improved datetime handling"""
        try:
            sync_record = {
                'data': data,
                'last_updated': datetime.now(UTC).isoformat()
            }
            result = await self.supabase._post("external_sync", sync_record)
            return result[0] if result else None
        except Exception as e:
            logger.error(f"Error syncing external data: {str(e)}")
            raise

    async def log_integration_event(self, event_type: str, details: Dict[str, Any]) -> Dict[str, Any]:
        """Log integration event with improved datetime handling"""
        try:
            event_record = {
            ...
            result = await self.supabase._post("integration_events", event_record)
            return result[0] if result else None
        except Exception as e:
    ...
    async def get_integration_status(self, integration_id: str) -> Optional[Dict[str, Any]]:
        """Get integration status with improved error handling"""
        try:
            params = {"integration_id": f"eq.{integration_id}", "select": "*"}
            status_records = await self.supabase._get("integration_status", params)
            return status_records[0] if status_records else None
        except Exception as e:
            logger.error(f"Error getting integration status: {str(e)}")
            return None

    async def update_integration_status(self, integration_id: str, status: str, details: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Update integration status with improved datetime handling"""
        try:
            status_data = {
                'status': status,
                'details': details,
            ...
            result = await self.supabase._patch("integration_status", status_data, params)
            return result[0] if result else None
        except Exception as e:
