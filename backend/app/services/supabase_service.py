...
class SupabaseService:
    ...
    async def _get(self, table: str, params: dict) -> Optional[List[dict]]:
        """Execute GET request with improved error handling and timeout"""
        try:
            ...
            logger.error(f"HTTP error in _get for table {table}: {e.response.status_code} - {e.response.text}")
            raise
        except ConnectError as e:
            logger.error(f"Connection error in _get for table {table}: {str(e)}")
            raise
        except TimeoutException as e:
            logger.error(f"Timeout error in _get for table {table}: {str(e)}")
            raise
        except Exception as e:
            logger.error(f"Unexpected error in _get for table {table}: {str(e)}")
            raise

    async def _post(self, table: str, data: dict) -> Optional[List[dict]]:
        """Execute POST request with improved error handling and timeout"""
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            logger.error(f"HTTP error in _post for table {table}: {e.response.status_code} - {e.response.text}")
            raise
        except ConnectError as e:
            logger.error(f"Connection error in _post for table {table}: {str(e)}")
            raise
        except TimeoutException as e:
            logger.error(f"Timeout error in _post for table {table}: {str(e)}")
            raise
        except Exception as e:
            logger.error(f"Unexpected error in _post for table {table}: {str(e)}")
            raise

    async def _patch(self, table: str, data: dict, params: dict) -> Optional[List[dict]]:
        """Execute PATCH request with improved error handling and timeout"""
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            logger.error(f"HTTP error in _patch for table {table}: {e.response.status_code} - {e.response.text}")
            raise
        except ConnectError as e:
            logger.error(f"Connection error in _patch for table {table}: {str(e)}")
            raise
        except TimeoutException as e:
            logger.error(f"Timeout error in _patch for table {table}: {str(e)}")
            raise
        except Exception as e:
            logger.error(f"Unexpected error in _patch for table {table}: {str(e)}")
            raise

    async def _delete(self, table: str, params: dict) -> bool:
        """Execute DELETE request with improved error handling and timeout"""
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            logger.error(f"HTTP error in _delete for table {table}: {e.response.status_code} - {e.response.text}")
            raise
        except ConnectError as e:
            logger.error(f"Connection error in _delete for table {table}: {str(e)}")
            raise
        except TimeoutException as e:
            logger.error(f"Timeout error in _delete for table {table}: {str(e)}")
            raise
        except Exception as e:
            logger.error(f"Unexpected error in _delete for table {table}: {str(e)}")
            raise

    async def verify_clerk_token(self, token: str) -> Optional[Dict[str, Any]]:
        """Verify Clerk JWT token with improved error handling"""
        try:
            # For development/testing, if CLERK_JWT_ISSUER is not set, 
            # we'll use a simplified token verification approach
            if not self.clerk_jwt_issuer:
                logger.warning("CLERK_JWT_ISSUER not set, using simplified token verification")
                # In development, accept any non-empty token as valid
                if token and len(token) > 10:  # Basic validation that token exists and has some length
                    ...
                    try:
                        ...
                        return {
                            "sub": decoded.get("sub", "dev_user"),
                            "email": decoded.get("email", "dev@example.com"),
                            "clerk_user_id": decoded.get("sub", "dev_user")
                        }
                    except Exception as e:
            ...
            # If CLERK_JWT_ISSUER is set, use proper verification
    ...
    async def get_user_by_clerk_id(self, clerk_user_id: str) -> Optional[Dict[str, Any]]:
        """Get user by Clerk ID with improved error handling"""
        try:
    ...
    async def create_user_from_clerk(self, clerk_user_data: Dict[str, Any]) -> Dict[str, Any]:
        """Create user from Clerk data with improved error handling"""
        try:
            user_data = {
    ...
    async def get_user_preferences(self, clerk_user_id: str) -> Dict[str, Any]:
        """Get user preferences with improved error handling and defaults"""
        try:
    ...
    async def update_user_preferences(self, clerk_user_id: str, preferences: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            prefs_data['created_at'] = datetime.now(UTC).isoformat()
    ...
    async def get_latest_career_report(self, clerk_user_id: str) -> Optional[Dict[str, Any]]:
        """Get latest career report with improved error handling"""
        try:
            user = await self.get_user_by_clerk_id(clerk_user_id)
            if not user:
                return None
    ...
    async def save_career_report(self, clerk_user_id: str, report_data: Dict[str, Any]) -> Dict[str, Any]:
        """Save career report with improved error handling"""
        try:
            user = await self.get_user_by_clerk_id(clerk_user_id)
            if not user:
                raise ValueError("User not found")
