        ...
        for attempt in range(max_retries):
            try:
                ...
                if "error" in response:
                    raise AIResponseError(f"Gemini API error: {response['error']}")
                
                # Check for required fields
                candidates = response.get("candidates", [])
                if not candidates:
                    raise AIResponseError("No candidates in Gemini API response")
                
                content = candidates[0].get("content", {})
                if not content:
                    raise AIResponseError("No content in Gemini API response")
                
                parts = content.get("parts", [])
                if not parts:
                    raise AIResponseError("No parts in Gemini API response")
                
                text = parts[0].get("text", "")
                if not text:
                    raise AIResponseError("No text in Gemini API response")
